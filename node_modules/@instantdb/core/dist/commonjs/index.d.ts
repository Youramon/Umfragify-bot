...
import { StoreInterface, type StoreInterfaceClass, type StoreInterfaceStoreName } from './utils/PersistedObject.ts';
...
import type { Query, QueryResponse, InstaQLResponse, PageInfoResponse, Exactly, InstantObject, InstaQLParams, InstaQLOptions, InstaQLQueryParams, InstaQLEntity, InstaQLEntitySubquery, InstaQLResult, InstaQLFields, ValidQuery, Cursor, Order, InstaQLQueryEntityResult } from './queryTypes.ts';
...
import type { CheckMagicCodeParams, CheckMagicCodeResponse, ExchangeCodeForTokenParams, SendMagicCodeParams, SendMagicCodeResponse, SignInWithIdTokenParams, VerifyMagicCodeParams, VerifyResponse } from './authAPI.ts';
import { InstantAPIError, type InstantIssue } from './utils/fetch.js';
import { InstantError } from './InstantError.ts';
import { SSEConnection, EventSourceType, type EventSourceConstructor } from './Connection.ts';
import { CallbackEventType as SyncTableCallbackEventType } from './SyncTable.ts';
import type { SyncTableCallback, CallbackEvent as SyncTableCallbackEvent, InitialSyncBatch as SyncTableInitialSyncBatch, InitialSyncComplete as SyncTableInitialSyncComplete, SyncTransaction as SyncTableSyncTransaction, LoadFromStorage as SyncTableLoadFromStorage, SetupError as SyncTableSetupError } from './SyncTable.ts';
import { InstantStream, InstantWritableStream, InstantReadableStream, ReadableStreamCtor, WritableStreamCtor } from './Stream.ts';
import { type InfiniteQueryCallbackResponse, type InfiniteQuerySubscription, getInfiniteQueryInitialSnapshot } from './infiniteQuery.ts';
type ExactlyOne<T> = {
...
export type InstantConfig<S extends InstantSchemaDef<any, any, any>, UseDates extends boolean = false> = {
    appId: string;
    schema?: S;
    websocketURI?: string;
...
export type GetPresence<PresenceShape> = <Keys extends keyof PresenceShape>(opts: PresenceOpts<PresenceShape, Keys>) => PresenceResponse<PresenceShape, Keys> | null;
...
type InstaQLLifecycleState<Schema, Q, UseDates extends boolean = false> = (InstaQLSubscriptionState<Schema, Q, UseDates> & {
    isLoading: boolean;
}) | {
    isLoading: true;
    data: undefined;
...
declare class Auth {
     ...
     *       .catch((err) => console.error(err.body?.message))
     */
    signInWithMagicCode: (params: CheckMagicCodeParams) => Promise<CheckMagicCodeResponse>;
    /**
     ...
     *
     */
    signInWithIdToken: (params: SignInWithIdTokenParams) => Promise<CheckMagicCodeResponse>;
    /**
     * Sign in with the id_token from an external provider like Google
     *
     * @see https://instantdb.com/docs/auth
     ...
     *
     */
    exchangeOAuthCode: (params: ExchangeCodeForTokenParams) => Promise<CheckMagicCodeResponse>;
    /**
...
declare class InstantCoreDatabase<Schema extends InstantSchemaDef<any, any, any>, UseDates extends boolean = false> implements IInstantDatabase<Schema> {
     ...
     *  });
     */
    subscribeQuery<Q extends ValidQuery<Q, Schema>, UseDatesLocal extends boolean = UseDates>(query: Q, cb: (resp: InstaQLSubscriptionState<Schema, Q, UseDatesLocal>) => void, opts?: InstaQLOptions): () => void;
    /**
     ...
     * });
     */
    subscribeInfiniteQuery<Q extends ValidQuery<Q, Schema>>(query: Q, cb: (resp: InfiniteQueryCallbackResponse<Schema, Q, UseDates>) => void, opts?: InstaQLOptions): InfiniteQuerySubscription;
    /**
     ...
     * room.leaveRoom();
     */
    joinRoom<RoomType extends string & keyof RoomsOf<Schema>>(roomType?: RoomType, roomId?: string, opts?: {
     ...
     */
    _syncTableExperimental<Q extends ValidQuery<Q, Schema>>(query: ExactlyOne<Q>, cb: SyncTableCallback<Schema, Q, UseDates>): (opts?: {
...
declare function init<Schema extends InstantSchemaDef<any, any, any> = InstantUnknownSchema, UseDates extends boolean = false>(config: Omit<InstantConfig<Schema, UseDates>, 'useDateObjects'> & {
    useDateObjects?: UseDates;
}, Store?: any, NetworkListener?: any, versions?: {
    [key: string]: string;
}, EventSourceImpl?: any): InstantCoreDatabase<Schema, UseDates>;
/**
 * @deprecated
...
export { init, init_experimental, id, tx, txInit, lookup, validateQuery, QueryValidationError, validateTransactions, parseSchemaFromJSON, TransactionValidationError, FrameworkClient, Reactor, InstantAPIError, setInstantWarningsEnabled, i, getOps, coerceQuery, weakHash, coerceToDate, IndexedDBStorage, WindowNetworkListener, InstantCoreDatabase, Auth, Storage, Streams, version, InstantError, getInfiniteQueryInitialSnapshot, SyncTableCallbackEventType, type IDatabase, type RoomSchemaShape, type Query, type QueryResponse, type InstaQLResponse, type PageInfoResponse, type InstantObject, type Exactly, type TransactionChunk, type AuthState, type ConnectionStatus, type User, type AuthToken, type TxChunk, type SubscriptionState, type InstaQLSubscriptionState, type LifecycleSubscriptionState, type InstaQLLifecycleState, type PresenceOpts, type PresenceSlice, type PresenceResponse, type PresencePeer, type InstaQLParams, type ValidQuery, type Cursor, type InstaQLOptions, type InstaQLQueryParams, type InstantQuery, type InstantQueryResult, type InstantSchema, type InstantEntity, type InstantSchemaDatabase, type InstaQLFields, type Order, type InstaQLQueryEntityResult, type InfiniteQueryCallbackResponse, type InfiniteQuerySubscription, type AttrsDefs, type CardinalityKind, type DataAttrDef, type EntitiesDef, type InstantUnknownSchemaDef, type EntitiesWithLinks, type EntityDef, type RoomsDef, type InstantGraph, type LinkAttrDef, type LinkDef, type LinksDef, type ResolveAttrs, type ValueTypes, type RoomsOf, type PresenceOf, type TopicsOf, type TopicOf, type InstaQLEntity, type InstaQLResult, type InstaQLEntitySubquery, type InstantSchemaDef, type InstantUnknownSchema, type IInstantDatabase, type BackwardsCompatibleSchema, type InstantRules, type UpdateParams, type LinkParams, type CreateParams, type RuleParams, type InstantDBAttr, type InstantDBAttrOnDelete, type InstantDBCheckedDataType, type InstantDBIdent, type InstantDBInferredType, type CheckMagicCodeParams, type CheckMagicCodeResponse, type ExchangeCodeForTokenParams, type SendMagicCodeParams, type SendMagicCodeResponse, type SignInWithIdTokenParams, type VerifyMagicCodeParams, type VerifyResponse, type FileOpts, type UploadFileResponse, type DeleteFileResponse, SSEConnection, type EventSourceType, type FrameworkConfig, type EventSourceConstructor, InstantStream, type WritableStreamCtor, type ReadableStreamCtor, type CreateReadStreamOpts, type CreateWriteStreamOpts, type InstantWritableStream, type InstantReadableStream, type SyncTableCallback, type SyncTableCallbackEvent, type SyncTableInitialSyncBatch, type SyncTableInitialSyncComplete, type SyncTableSyncTransaction, type SyncTableLoadFromStorage, type SyncTableSetupError, type InstantIssue, StoreInterface, type StoreInterfaceClass, type StoreInterfaceStoreName, createInstantRouteHandler, };
/** @deprecated Use StoreInterface instead */
export declare const StorageInterface: typeof StoreInterface;
